...
int WinkelGewaehlt[36]; //Wegen Programmspeicher auf 36 begrenzt, eigentlich sollen 360° möglich sein
...
void turnleftAuto(){ //Void zum automatischen Drehen des Steppers gegen den Uhrzeigersinn
  while(StepsForAutomation<=((-1*(WinkelG/AnzahlA)*(2000/WinkelM)))){
  myStepper.step(-1);
  Serial.print("steps:");
  Serial.println(stepCount);
  stepCount--;
...
void turnrightAuto(){ //Void zum automatischen Drehen des Steppers im Uhrzeigersinn
  myStepper.step(1);
  Serial.print("steps:");
  Serial.println(stepCount);
  stepCount++;
...
lcd.setCursor(0,1);
lcd.print("1. L / 2. R");
if (modestate==8&&IrReceiver.decode()==true&&IrReceiver.decodedIRData.command==0x44){
turnleftAuto();
}
if (modestate==8&&IrReceiver.decode()==true&&IrReceiver.decodedIRData.command==0x40){
turnrightAuto();
}
modestate=10;
}
...
if(IrReceiver.decode()==true&&IrReceiver.decodedIRData.command==0x46&&modestate==2){
...
if(IrReceiver.decodedIRData.command==0x15&&status==0){
turnrightstep();
}
